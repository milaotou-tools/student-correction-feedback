  ...
  return (
    <section className="min-w-0 rounded-md border border-[#E8E6E1] bg-white">
      ...
      <div className="p-5">
        ...
        <div>
          <h3 className="mb-3 font-extrabold text-[#141413]">适用学生</h3>
          <div className="flex flex-wrap gap-2">
            {group.students.map((student) => (
              <span
                key={`${student.className}-${student.studentId}-${student.name}`}
                title={formatStudentMeta(student)}
                className="inline-flex rounded-md border border-[#E8E6E1] bg-white px-3 py-2 text-sm font-bold text-[#141413]"
              >
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#FAF9F6] px-4 py-6 sm:px-5 sm:py-8">
      <section className="mx-auto w-full max-w-7xl">
        <div className="mb-6 flex flex-col gap-4 border-b border-[#E8E6E1] pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-bold text-[#141413]">仅用于教师沟通准备</p>
            <h1 className="mt-1 text-3xl font-extrabold text-[#141413]">家长沟通话术</h1>
            <p className="mt-2 text-[#6B7280]">相似情况合并成同一套话术，分为必须沟通和正向反馈。</p>
          </div>
        ...
        {messageGroups.length === 0 ? (
          <div className="rounded-md border border-[#E8E6E1] bg-white px-4 py-10 text-center text-[#6B7280]">本次暂无需要家长沟通反馈的学生。</div>
        ) : (
          <div className="space-y-8">
            <section>
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-2xl font-extrabold text-[#141413]">必须沟通</h2>
                  <p className="mt-1 text-sm text-[#6B7280]">低分、明显退步、边缘下滑优先处理。</p>
                </div>
                <CopyButton text={formatSectionText(`必须沟通（${mustStudentCount}人）`, mustGroups)} label="复制必须沟通" />
              </div>
              <div className="space-y-6">
                {mustGroups.length === 0 ? (
                  <div className="rounded-md border border-[#E8E6E1] bg-white px-4 py-8 text-center text-[#6B7280]">本次暂无必须沟通学生。</div>
                ) : (
            ...
            <section>
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-2xl font-extrabold text-[#141413]">正向反馈</h2>
                  <p className="mt-1 text-sm text-[#6B7280]">进步明显的学生可选发给家长，主要用于肯定和鼓励。</p>
                </div>
                <CopyButton text={formatSectionText(`正向反馈（${positiveStudentCount}人）`, positiveGroups)} label="复制正向反馈" />
              </div>
              <div className="space-y-6">
                {positiveGroups.length === 0 ? (
                  <div className="rounded-md border border-[#E8E6E1] bg-white px-4 py-8 text-center text-[#6B7280]">本次暂无正向反馈学生。</div>
                ) : (
