...
import { optimizeTitleClinic, normalizeOutline } from "@/lib/title-clinic-ai";
...
function validatePayload(body: Record<string, unknown>) {
  ...
  if (!title && !outline.length) {
    return "请先上传 Word，或手动填写原标题和目录。";
  }

  if ((fullText + abstract + intro).length < 80) {
    return "请上传完整 Word，或补充摘要、引言、正文内容。";
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await optimizeTitleClinic(body);
