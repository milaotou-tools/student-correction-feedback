...
async function readJsonResponse<T>(response: Response): Promise<T & { error?: string }> {
  ...
  try {
    ...
    const summary = text
      ...
      .trim()
      .slice(0, 120);
    throw new Error(`服务返回了非 JSON 响应（HTTP ${response.status}）。${summary || "多半是线上代理超时或服务错误，请稍后重试。"}`);
  }
}
...
export function TitleClinicApp() {
  ...
  async function checkHealth() {
    ...
    try {
      ...
      const data = await readJsonResponse<HealthStatus>(response);
      if (!response.ok || !data.ok) throw new Error(data && "error" in data ? String(data.error) : "API unavailable");
      setHealth(data);
    } catch (error) {
      setHealth(null);
  ...
  async function generate() {
    ...
    try {
      ...
      const payload = await readJsonResponse<OptimizeResult & { error?: string }>(response);
